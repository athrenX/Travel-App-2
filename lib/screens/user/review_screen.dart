...
class _ReviewScreenState extends State<ReviewScreen> {
  ...
  bool _isLoadingExistingReview = true; // State untuk loading review yang sudah ada
  ...
  Future<void> _fetchExistingReview() async {
    ...
    if (authProvider.token == null || authProvider.token!.isEmpty) {
      if (mounted) { // Periksa mounted sebelum setState
        setState(() {
    ...
    try {
      final fetchedReview =
          await reviewProvider.fetchReviewByOrder(widget.pemesanan.id, authProvider.token!);
      
      if (mounted) { // <--- TAMBAHKAN INI DI SINI
        setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Container(
        ...
        child: SafeArea(
          child: _isLoadingExistingReview
  ...
  void _submitReview() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (authProvider.user != null) {
        userName = authProvider.user?.nama ??
      ...
      try {
        if (_existingReview == null) {
          ...
          await reviewProvider.updateReview(
            _existingReview!.id, // reviewId (String)
            _commentController.text, // comment
            _rating, // ratingValue
            token,
          );
        ...
        if (mounted) { // <--- DAN JUGA DI SINI
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Gagal menyimpan ulasan. Silakan coba lagi. Error: ${e.toString()}'),
              backgroundColor: Colors.red,
            ),
          );
        }
